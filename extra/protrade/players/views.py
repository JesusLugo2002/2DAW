...
def player_list(request: HttpRequest) -> JsonResponse:
    ...
    if position_query := request.GET.get('position'):
        players.filter(position=position_query)
    if team_query := request.GET.get('team'):
        players.filter(team__slug=team_query)
...
def add_player(request: HttpRequest):
    ...
    valid_positions = (pos for pos, value in Player.Position.choices)
    if request.data['position'] not in valid_positions:
